...
static size_t search_closest_next_space(const char *data, size_t index,
                                        size_t data_length) {
...
static size_t *count_data_parts(const char *data, size_t data_length,
  ...
  for (size_t i = 1; i < processors_count - 1; i++) {
    if (data_length - data_parts[i - 1] > data_part_size) {
      data_parts[i] = search_closest_next_space(
          data, data_part_size + data_parts[i - 1], data_length);
    } else {
...
static char *pthread_search_long_word(pthread_args_t *args) {
  char *longest_word =
      search_long_word(args->data, args->data_size, &args->word_len);
  return longest_word;
}

static size_t pthreads_create(pthread_t *threads, pthread_args_t *threads_args,
                              size_t pthread_count, const char *data,
                              size_t *data_parts) {
  if (pthread_count == 0) {
    return 0;
  }
...
char *file_long_word_search(const char *filename, size_t *word_length) {
  ...
  size_t success_pthread =
      pthreads_create(threads, threads_args, pthread_count, data, data_parts);
  ...
  if (success_pthread != 0) {
    longest_word =
        pthreads_join(threads, threads_args, success_pthread, &word_max_len);
  } else {
